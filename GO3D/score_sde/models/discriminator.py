...
conv3d = dense_layer.conv3d  # Change to 3D convolution
...
class DownConvBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            conv3d(in_channel, out_channel, kernel_size, padding=padding),
        )

        self.conv2 = nn.Sequential(
            conv3d(out_channel, out_channel, kernel_size, padding=padding, init_scale=0.)
        )
        ...
        self.skip = nn.Sequential(
            conv3d(in_channel, out_channel, 1, padding=0, bias=False),
        )

    def forward(self, input, t_emb):
        out = self.act(input)
        out = self.conv1(out)
        out += self.dense_t1(t_emb)[..., None, None, None]

        out = self.act(out)

        if self.downsample:
            out = up_or_down_sampling.downsample_3d(out, self.fir_kernel, factor=2)  # Change to 3D downsampling
            input = up_or_down_sampling.downsample_3d(input, self.fir_kernel, factor=2)  # Change to 3D downsampling
        out = self.conv2(out)
...
class Discriminator_small(nn.Module):
    ...
    def __init__(self, nc=3, ngf=64, t_emb_dim=128, act=nn.LeakyReLU(0.2), patch_size=1, use_local_loss=False, num_layers=4):
        ...
        self.start_conv = conv3d(nc, ngf * 2, 1, padding=0)
        ...
        self.final_conv = conv3d(ngf * 8 + 1, ngf * 8, 3, padding=1, init_scale=0.)
    ...
    def forward(self, x, t, x_t):
        x = rearrange(x, "n c (h p1) (w p2) (d p3) -> n (c p1 p2 p3) h w d", p1=self.patch_size, p2=self.patch_size, p3=self.patch_size)
        x_t = rearrange(x_t, "n c (h p1) (w p2) (d p3) -> n (c p1 p2 p3) h w d", p1=self.patch_size, p2=self.patch_size, p3=self.patch_size)
        ...
        t = out
        out = self.end_linear(out.sum(3))
        if self.use_local_loss:
            out2 = self.local_end_linear(t.permute(0, 3, 2, 1))
            return (out, out2)
...
class Discriminator_large(nn.Module):
    ...
    def __init__(self, nc=1, ngf=32, t_emb_dim=128, act=nn.LeakyReLU(0.2), patch_size=1, use_local_loss=False, num_layers=6):
        ...
        self.final_conv = conv3d(ngf * 8 + 1, ngf * 8, 3, padding=1)
        self.end_linear = dense(ngf * 8, 1)
        if use_local_loss:
            self.local_end_linear = dense(ngf * 8, 1)
    ...
    def forward(self, x, t, x_t):
        ...
        #print(x.shape, t.shape, x_t.shape)
