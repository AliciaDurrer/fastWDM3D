...
def upsample_conv_3d(x, w, k=None, factor=2, gain=1):
    ...
    # Define padding for 3D: (front, back) for depth, height, and width
    pad = (
        (p_d + 1) // 2 + factor - 1, p_d // 2,  # Padding for depth
        (p_h + 1) // 2 + factor - 1, p_h // 2,  # Padding for height
        (p_w + 1) // 2 + factor - 1, p_w // 2   # Padding for width
    )
...
def upsample_3d(x, k=None, factor=2, gain=1):
    ...
    if k is None:
        k = [1] * factor
    k = _setup_kernel_3d(k) * (gain * (factor ** 3))
    # Calculate padding for each dimension
    p = k.shape[0] - factor
    return upfirdn3d(x, torch.tensor(k, device=x.device),
