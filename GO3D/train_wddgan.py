...
def train(rank, gpu, args):
    ...
    parent_dir = "./saved_info/go3d/{}".format(name_dataset)
    ...
    logging.info(f'coeff: {coeff.__dict__}')
    logging.info(f'pos_coeff: {pos_coeff.__dict__}')
    logging.info(f'T: {T}')
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        for iteration, (x, y, mk) in enumerate(data_loader):
            ...
            real_data = torch.cat(
                [LLL0, LLH0, LHL0, LHH0, HLL0, HLH0, HHL0, HHH0, LLL1, LLH1, LHL1, LHH1, HLL, HLH1, HHL1, HHH1, LLL2,
                 LLH2, LHL2, LHH2, HLL2, HLH2, HHL2, HHH2], dim=1)

            real_data = real_data / np.sqrt(8.0)
            ...
            errG_total = rec_loss_IWT_full + rec_loss_IWT_masked
            ...
            if iteration % 10 == 0:
                if rank == 0:
                    print('epoch {} iteration{}, Total Loss: {}, Full Rec Loss: {}, Mask Rec Loss: {}'.format(
                        epoch, iteration, errG_total.item(), rec_loss_IWT_full.item(), rec_loss_IWT_masked.item()))

            writer.add_scalar('Loss/trainG_total', errG_total.item(), global_step)
            writer.add_scalar('Loss/rec_loss_full', rec_loss_IWT_full.item(), global_step)
            writer.add_scalar('Loss/rec_loss_mask', rec_loss_IWT_masked.item(), global_step)
            ...
            if (global_step - 1) % 500 == 0:
                ...
                if not args.use_pytorch_wavelet:
                    ...
                    fake_sample = iwt(
                        fake_sample[:, 0:1], fake_sample[:, 1:2], fake_sample[:, 2:3], fake_sample[:, 3:4],
                        fake_sample[:, 4:5], fake_sample[:, 5:6], fake_sample[:, 6:7], fake_sample[:, 7:8])
                ...
                writer.add_image('Images Real Voided /real_data_voided_0_0', real_data_0[0, :, idx_show_0, :, :],
                                 global_step - 1)
                writer.add_image('Images Real Mask /real_data_mask_0_0', real_data_1[0, :, idx_show_0, :, :],
                                 global_step - 1)
                writer.add_image('Images Real GT /real_data_t1n_0_0', real_data_2[0, :, idx_show_0, :, :],
                                 global_step - 1)
                writer.add_image('Images Real Voided /real_data_voided_0_1', real_data_0[0, :, :, idx_show_1, :],
                                 global_step - 1)
                writer.add_image('Images Real Mask /real_data_mask_0_1', real_data_1[0, :, :, idx_show_1, :],
                                 global_step - 1)
                writer.add_image('Images Real GT /real_data_t1n_0_1', real_data_2[0, :, :, idx_show_1, :],
                                 global_step - 1)
                writer.add_image('Images Real Voided /real_data_voided_0_2', real_data_0[0, :, :, :, idx_show_2],
                                 global_step - 1)
                writer.add_image('Images Real Mask /real_data_mask_0_2', real_data_1[0, :, :, :, idx_show_2],
                                 global_step - 1)
                writer.add_image('Images Real GT /real_data_t1n_0_2', real_data_2[0, :, :, :, idx_show_2],
                                 global_step - 1)

                writer.add_image('Images Fake Sample /fake_sample_0_0', fake_sample[0, :, idx_show_0, :, :],
                                 global_step - 1)
                writer.add_image('Images Fake Sample /fake_sample_0_1', fake_sample[0, :, :, idx_show_1, :],
                                 global_step - 1)
                writer.add_image('Images Fake Sample /fake_sample_0_2', fake_sample[0, :, :, :, idx_show_2],
                                 global_step - 1)
