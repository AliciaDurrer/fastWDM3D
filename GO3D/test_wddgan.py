...
def sample_and_test(args):
    ...
    save_dir = "./go3d_generated_samples/{}_{}".format(args.dataset, args.checkpoint_path.split('/')[-1].split('.')[0])
    ...
    netG.eval()
    logging.info(f'model used: {netG}')
    ...
    for iteration, (stacked_images, voided_image_full, starts, ends, labeled_masks, file_name) in enumerate(data_loader):
        ...
        voided_image_full = to_range_0_1(voided_image_full)
        ...
        for i, real_data in enumerate(stacked_images):
            ...
            real_data = torch.cat(
                [LLL0, LLH0, LHL0, LHH0, HLL0, HLH0, HHL0, HHH0, LLL1, LLH1, LHL1, LHH1, HLL, HLH1, HHL1, HHH1], dim=1)

            real_data = real_data / math.sqrt(8.0)
        ...
        elapsed_time_individual = end_time_individual - start_time_individual
        print(f"Sampled file {file_name[0].replace('t1n-voided', 't1n-inference')}, this individual sampling required: {elapsed_time_individual:.6f} seconds")
    ...
    logging.info(f"Average time per sample (including setting up process): {elapsed_time/num_sam:.6f} seconds")


    elapsed_time_sampling_only = end_time - start_actual_sampling
    print(f"Total elapsed time (sampling only): {elapsed_time_sampling_only:.6f} seconds")
    logging.info(f"Average time per sample (sampling only): {elapsed_time_sampling_only/num_sam:.6f} seconds")


    create_submission_adapted.adapt(input_data=args.datadir, samples_dir=save_dir, adapted_samples_dir=save_dir)
    logging.info(f"adapted submissions saved in: {save_dir}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='cuda',
                        help='GPU to use')
    parser.add_argument('--checkpoint_path', default='/home/user/model/checkpoint_file.pth')
