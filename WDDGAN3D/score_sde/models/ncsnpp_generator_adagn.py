...
class PixelNorm(nn.Module):
    ...
    def forward(self, input):
        return input / torch.sqrt(torch.mean(input ** 3, dim=1, keepdim=True) + 1e-8)
...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.all_resolutions = all_resolutions = [
            (config.image_size // self.patch_size) // (3 ** i) for i in range(num_resolutions)]
        ...
        channels = config.num_channels * self.patch_size** 3
    ...
    def forward(self, x, time_cond, z):
        # patchify
        x = rearrange(x, "n c (h p1) (w p2) (d p3) -> n (p1 p2 c) h w d",
        ...
        # unpatchify
        h = rearrange(h, "n (c p1 p2 p3) h w d -> n c (h p1) (w p2) (d p3)",
                      p1=self.patch_size, p2=self.patch_size, p3=self.patch_size)
...
class WaveletNCSNpp(NCSNpp):
    ...
    def __init__(self, config):
        ...
        if progressive != 'output_skip':
            channels = getattr(config, "num_out_channels", int(channels / 3))
            modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                        num_channels=in_ch, eps=1e-6))
            modules.append(conv3x3(in_ch, channels, init_scale=init_scale))
        ...
        mapping_layers = [dense(config.nz, z_emb_dim),
                          self.act, ]
        for _ in range(config.n_mlp):
            mapping_layers.append(dense(z_emb_dim, z_emb_dim))
            mapping_layers.append(self.act)
    ...
    def forward(self, x, time_cond, z):
        # patchify
        x = rearrange(x, "n c (h p1) (w p2) (d p3) -> n (c p1 p2 p3) h w d",
                      p1=self.patch_size, p2=self.patch_size, p3=self.patch_size)
        # timestep/noise_level embedding; only for continuous training
        ...
        if self.no_use_fbn:
            ...
            h, LLH, LHL, LHH, HLL, HLH, HHL, HHH = self.dwt(h)
            h = modules[m_idx](h / 2., temb, zemb)
            h = self.iwt(h * 2., LLH, LHL, LHH, HLL, HLH, HHL, HHH)
        m_idx += 1
