...
def ncsn_conv1x1(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1., padding=0):
  ...
  conv = nn.Conv3d(in_planes, out_planes, kernel_size=(1,1,1), stride=stride, bias=bias, dilation=dilation,
                   padding=padding)
...
def ddpm_conv1x1(in_planes, out_planes, stride=1, bias=True, init_scale=1., padding=0, groups=1):
  ...
  conv = nn.Conv3d(in_planes, out_planes, kernel_size=1, stride=stride, padding=padding, bias=bias, groups=1)
...
class MSFBlock(nn.Module):
  ...
  def forward(self, xs, shape):
    ...
    for i in range(len(self.convs)):
      h = self.convs[i](xs[i])
      h = F.interpolate(h, size=shape, mode='trilinear', align_corners=True)
      sums += h
    return sums
  ...
  def __init__(self, input_dim, output_dim, kernel_size=3, biases=True, adjust_padding=False):
    ...
    if not adjust_padding:
      ...
      self.conv = nn.Sequential(
        nn.ZeroPad3d((1, 0, 1, 0, 1, 0)),
        conv
      )

  def forward(self, inputs):
    output = self.conv(inputs)
    output = sum([output[:, :, ::2, ::2, ::2], output[:, :, 1::2, ::2, ::2],
                  output[:, :, ::2, 1::2, ::2], output[:, :, 1::2, 1::2, ::2],
                  output[:, :, ::2, ::2, 1::2], output[:, :, 1::2, ::2, 1::2],
                  output[:, :, ::2, 1::2, 1::2], output[:, :, 1::2, 1::2, 1::2]]) / 8.
    return output
...
class UpsampleConv(nn.Module):
  ...
  def forward(self, inputs):
    output = inputs
    output = torch.cat([output] * 8, dim=1)  # Repeat for 3D
...
class NIN(nn.Module):
  ...
  def forward(self, x):
    x = x.permute(0, 2, 3, 4, 1)  # Change to 5D for 3D input
    y = contract_inner(x, self.W) + self.b
    return y.permute(0, 4, 1, 2, 3)  # Change back to original order
...
class AttnBlock(nn.Module):
  ...
  def forward(self, x):
    B, C, D, H, W = x.shape  # 3D input
    h = self.GroupNorm_0(x)
    q = self.NIN_0(h)
    k = self.NIN_1(h)
    v = self.NIN_2(h)

    w = torch.einsum('bcdhw,bcij->bhijw', q, k) * (int(C) ** (-0.5))
    w = w.view(B, D, H, W, H * W)
    w = F.softmax(w, dim=-1)
    w = w.view(B, D, H, W, H, W)
    h = torch.einsum('bhijw,bcij->bcdhw', w, v)
    h = self.NIN_3(h)
    return x + h
...
class Downsample(nn.Module):
  ...
  def forward(self, x):
    ...
    if self.with_conv:
      x = F.pad(x, (0, 1, 0, 1, 0, 1))  # Emulate 'SAME' padding
      x = self.Conv_0(x)
    else:
