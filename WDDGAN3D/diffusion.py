...
def sample_from_model(coefficients, generator, n_time, x_init, T, real_data, opt):
    x = x_init
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            if x.shape[1] != 24:
                x = torch.cat((real_data[:, :16, :, :, :], x), dim=1)
            x_0 = generator(x, t_time, latent_z)
            #x_0 = torch.cat((real_data[:, :16, :, :, :], x_0), dim=1)
            if x.shape[1] == 24:
                x = x[:, 16:, :, :, :]
...
def sample_from_model_test(coefficients, generator, n_time, x_init, T, real_data, opt):
    x = x_init
    with torch.no_grad():
        for i in reversed(range(n_time)):
            t = torch.full((x.size(0),), i, dtype=torch.int64).to(x.device)
